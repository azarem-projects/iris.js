...
function addId(str: string, prefix: string, _prefix: string) {
  ...
  for (var i = 0; i < split.length; i++) {
    ...
    if (!_split.includes(' ') && !_split.includes("'") && !_split.includes("\"") && !_split.includes("`")) {
  ...
  if (components) {
    for (var i = 0; i < components.length; i++) {
      str = str.replace(`${components[i]}, null`, `${components[i]},{___mod:0,_id:'${id++}',key:'${key++}'}`);
      str = str.replace(`${components[i]},null`, `${components[i]},{___mod:0,_id:'${id++}',key:'${key++}'}`);
      str = str.replace(`${components[i]},{`, `${components[i]},{___mod:0,_id:'${id++}',key:'${key++}',`);
      str = str.replace(`${components[i]}, {`, `${components[i]},{___mod:0,_id:'${id++}',key:'${key++}',`);
    }
  }
